...
function Contributors(){
  ...
  const intialFormState = {idMovie: null, role: '', idPerson: null, person: {idPerson: null, name: ''},
};
  const [ contributors, setContributors ] = useState(dataContributors);
...
;
  ...
  //       "Accept": "application/json"
  //     }, 
  //     body: JSON.stringify(editedContributor)
  //   };
  
  //   fetch(`http://localhost:8080/api/contributor/${idPerson}`, init)
  //     .then(response => {
  //       if (response.status === 204){
  //         // setEditing(false)
  //         setContributors(contributors.map(contributor => (contributor.idPerson === idPerson ? editedContributor : contributor)));
  //       } else if (response.status === 404){
  //         return Promise.reject("Contributor Not Found");
  //       } else {
  //         return Promise.reject(`Delete Failed with the Status: ${response.status}`);
